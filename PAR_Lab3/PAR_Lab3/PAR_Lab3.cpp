...
public:
    atomic<int>* threadsInWaitPtr = nullptr;
    ...
    bool popTask(Task& task)
    {
        ...
        if (threadsInWaitPtr) (*threadsInWaitPtr)++;
...
private:
    ...
    atomic<int> threadsInWait{ 0 };
    atomic<long long> waitSamplesSum{ 0 };
    atomic<int> waitSamplesCount{ 0 };
    thread waitSamplerThread;
...
public:
    ThreadPool()
    {
        taskQueueOne.threadsInWaitPtr = &threadsInWait;
        taskQueueTwo.threadsInWaitPtr = &threadsInWait;
        ...
        waitSamplerThread = thread([this]() {
            while (!isShutdown) {
                this_thread::sleep_for(milliseconds(100));
                waitSamplesSum += threadsInWait.load();
                waitSamplesCount++;
            }
            });
    }
    ...
    void shutdownPool()
    {
        ...
        if (waitSamplerThread.joinable()) waitSamplerThread.join();
    }
    ...
    void printStats() const
    {
        ...
        if (waitSamplesCount > 0)
        {
            cout << "Avg threads in wait: "
                << static_cast<double>(waitSamplesSum) / waitSamplesCount << endl;
        }
